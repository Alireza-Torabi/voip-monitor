...
Phase 2 Tasks 1–5 target Node.js 24.21.0 and npm 11.19.0. `.node-version` pins the Node patch used for validation; `package.json` limits the supported major to Node 24. Use a trusted official Node distribution or an approved host package source. Do not install tooling on a legacy PBX.
...
`--ignore-scripts` was verified for this lockfile; it avoids install-time package scripts. Recheck it if dependencies change. The frontend development server is started with `npm run dev -w frontend`. From the repository root, build all workspaces with `npm run build` and start the backend with `npm run start -w backend`. Its routes include `GET /health`, `GET /ready`, `GET /setup/status`, `POST /setup/admin`, `POST /auth/login`, `GET /auth/me`, and `POST /auth/logout`. The backend requires a writable SQLite database path and a protected secret directory; startup creates a master key and, before first-admin setup, a bootstrap token there when safe to do so. It binds to loopback by default; `APP_HOST` and `APP_PORT` may be set for development. No PBX connection or monitoring data exists.
