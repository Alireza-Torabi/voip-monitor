...
`ProviderRuntimeManager` now owns at most one provider lifecycle per enabled PBX. With `APP_PBX_NETWORK_MODE=plain_tcp`, it creates a provider asynchronously, reconnects with bounded exponential backoff and jitter after failures, and reconciles every 45 seconds by default. Profile updates replace the managed instance and deletion stops it. With the default `disabled` mode, no real provider factory exists and no PBX network socket can be opened. Provider failures never make application readiness fail. Plain AMI TCP is not treated as sufficient protection on an untrusted network; TLS or a protected private/tunneled deployment path remains a production prerequisite.
