...
Status: Phase 2 Task 16 extends the internal telephony state engine with capability-aware queue/member/caller state. It consumes independent ordered channel, endpoint, trunk, and queue snapshots plus normalized AMI events to maintain deterministic in-memory channel/call/endpoint/trunk/queue state with revision and synchronization freshness. PBX networking remains disabled by default outside explicit validation/runtime opt-in.
