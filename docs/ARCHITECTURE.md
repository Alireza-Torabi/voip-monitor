...
The telephony-state engine will consume the two Task 11 runtime feeds: authoritative channel snapshots and normalized live events. It must subscribe before provider start, buffer live events until the first snapshot, apply the buffer idempotently, and use later snapshots to repair drift. Task 11 provides the feeds but not the reducer/state machine. One backend state engine per PBX will later emit ordered revisions. Authenticated REST handles management and initial reads; authenticated WebSocket will send an initial per-instance state plus versioned updates without increasing AMI work.
