...
The shared `PbxProvider` contract defines `connect`, `disconnect`, `discover`, `getCapabilities`, `getHealth`, `subscribeEvents`, and `reconcile`. Task 7 established the `AmiTransport` seam and network boundary. Task 8 added `TcpAmiTransport`, `NodeAddressResolver`, and `AsteriskProvider`. Task 10 adds the normalized event seam. A hostname is resolved once, every result is checked, and TCP connects to the selected numeric address so the transport cannot trigger a second DNS lookup.

The TCP transport validates the manager banner, frames actions with CRLF headers, injects an internal ActionID, allows one outstanding action at a time, bounds response time and buffered input, and publishes parsed event frames to internal listeners. `AsteriskProvider` normalizes a deliberately bounded event subset instead of forwarding raw AMI payloads: channel lifecycle/state, dial lifecycle, bridge membership, and chan_sip peer status. Managed runtime entries subscribe before connect, so their `Login` requests events; one-shot connection verification has no event subscriber and keeps events disabled. Capability fields remain `UNKNOWN` rather than inferring support before later permission/action discovery. Provider health reports safe bounded error codes only.

`ProviderRuntimeManager` owns at most one provider lifecycle per enabled PBX. With `APP_PBX_NETWORK_MODE=plain_tcp`, it creates a provider asynchronously, reconnects with bounded exponential backoff and jitter after failures, and reconciles every 45 seconds by default. Profile updates replace the managed instance and deletion stops it. The runtime also exposes one internal normalized-event subscription boundary for the future state engine; subscribers do not create PBX connections. With the default `disabled` mode, no real provider factory exists and no PBX network socket can be opened. Provider failures never make application readiness fail. Plain AMI TCP is not treated as sufficient protection on an untrusted network; TLS or a protected private/tunneled deployment path remains a production prerequisite.

## Realtime and failure handling

The next telephony-state work will consume normalized provider events plus authoritative reconciliation snapshots; Task 10 does not yet build that state. One backend state engine per PBX will emit ordered revisions. Authenticated REST handles management and initial reads. Authenticated WebSocket sends an initial per-instance snapshot and versioned updates. A reconnect or revision gap triggers a fresh snapshot. Slow clients are bounded or disconnected without increasing AMI work. Reconnect and stale states remain visible to users.
