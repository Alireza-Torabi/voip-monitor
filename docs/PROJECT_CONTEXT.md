...
Status: Phase 2 Task 14 endpoint/registration state foundation is implemented on `feature/endpoint-registration-state`, 2026-09-25. Task 13 is merged into `main` by PR #14. The backend now builds deterministic in-memory current channel/call plus chan_sip endpoint state from independent authoritative snapshots and ordered normalized live events, with capability and synchronization freshness. No telephony state is exposed over REST/WebSocket yet. License: Apache-2.0. This is a public, reusable VoIP monitoring application hosted separately from the PBX. The first provider targets Asterisk and FreePBX-based Asterisk.

## Repository state

The public foundation repository is established on `main`, tracking `origin/main`. Task 13 is merged. The current feature branch extends the provider/state boundary with chan_sip endpoint snapshots from `SIPpeers`, normalized PeerStatus registration/reachability updates, endpoint capability reporting, and independent endpoint snapshot ordering. The reducer fails closed when endpoint authority is unavailable, preserves partial live-update dimensions, marks current state stale across connection loss, and clears state on provider profile reset/removal. Application PBX networking remains disabled unless the operator explicitly sets `APP_PBX_NETWORK_MODE=plain_tcp`; Task 13 uses synthetic/loopback validation only. The configured `origin` points to the user-supplied GitHub repository via SSH.
