...
Status: Phase 7 Task 27 is implemented on `feature/security-event-api-realtime`, 2026-09-26. Task 26 is merged into `main` as PR #27. The backend now has bounded Asterisk AMI authentication-event normalization, PBX-scoped security current/history persistence, and authenticated current/history HTTP plus PBX-scoped SSE realtime delivery. Synthetic tests cover normalization, source validation, migration 8, persistence semantics, authenticated API behavior, range/limit validation, same-origin stream protection, and realtime delivery. No production log access or real PBX connection was used for Task 27. Alert evaluation, notifications, dashboard, and broader security sources remain future work. License: Apache-2.0. This is a public, reusable VoIP monitoring application hosted separately from the PBX. The first provider targets Asterisk and FreePBX-based Asterisk.
