...
Status: Phase 7 Task 25 is implemented on `feature/security-monitoring-foundation`, 2026-09-26. Task 24 is merged into `main` as PR #25. The backend now has a bounded provider-neutral security-event contract, Asterisk AMI `SecurityEvent` normalization for authentication outcomes, a provider/runtime subscription boundary, and fail-closed event validation. Synthetic tests cover normalization, capability activation, AMI event enablement, source validation, and exclusion of raw account/network/request fields. No production log access or real PBX connection was used for Task 25. Security-event persistence/history, authenticated API/realtime exposure, dashboard, and broader security sources remain future work. License: Apache-2.0. This is a public, reusable VoIP monitoring application hosted separately from the PBX. The first provider targets Asterisk and FreePBX-based Asterisk.

## Repository state

The public foundation repository is established on `main`, tracking `origin/main`. Task 24 is merged as PR #25. The current feature branch adds the first bounded security-monitoring source boundary. SSH metadata is PBX-scoped in SQLite; password/private-key/passphrase material uses the existing encrypted PBX secret store and never appears in safe configuration records. Trust is pinned-only using canonical OpenSSH SHA-256 fingerprints with timing-safe verification; no TOFU/accept-new path exists. The concrete transport resolves through an injected resolver once, validates all returned addresses through the shared network boundary, selects an approved address, and executes only typed restricted commands with bounded streaming output and timeout handling. `SystemMetricsRuntime` owns one SSH collector lifecycle per configured PBX, requires encrypted SSH credentials, publishes PBX-scoped SSH source health, uses bounded exponential failure backoff, and persists samples through PBX-scoped current/history repositories. Current state is monotonic by observation timestamp; history is duplicate-safe and pruned transactionally with a bounded seven-day default retention window. The production factory is gated by `APP_PBX_NETWORK_MODE=plain_tcp`; the default `disabled` mode creates no SSH source runtime. The public security boundary now normalizes only selected Asterisk AMI authentication events, strips account/address/request fields, and validates PBX/source identity and timestamps before delivery. Security persistence/API/UI and broader real-host compatibility remain future work. The configured `origin` points to the user-supplied GitHub repository via SSH.
