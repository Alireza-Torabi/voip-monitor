...
Managed runtime entries subscribe before AMI login; one-shot connection verification has no provider-event subscriber and therefore keeps AMI events disabled. Runtime subscribers receive cloned normalized events and cannot create additional PBX connections. No HTTP/WebSocket event endpoint, persistence, or telephony state engine exists yet. The current AMI parser stores one value per header name, so repeated AMI headers are not preserved.

## Phase 2 Task 11 snapshot review

AMI event-list responses are correlated by the internally generated ActionID. Matching list items and the completion event are consumed by the snapshot request and are not emitted as live events; unrelated unsolicited events continue through the live-event path. Cancelled lists, timeouts, unexpected correlated events, and inconsistent `ListItems` counts are rejected so partial state is not accepted as authoritative.

The channel snapshot intentionally retains only stable channel ID, channel name, linked ID, state, and bridge ID. Caller/connected-line identity, dialplan/application payloads, source addresses, and arbitrary AMI fields are excluded. Snapshot-only failure leaves an otherwise connected provider degraded and retries on the normal reconciliation interval instead of repeatedly reconnecting. Real deployment credentials remain encrypted/runtime-only and must never be added to Git or CI. One controlled real Asterisk 13.x compatibility gate has now passed; broader release/version compatibility remains separate validation work.

## Phase 2 Task 12 controlled real-PBX verification

Real compatibility inputs are entered only on the monitoring host by `scripts/setup-real-pbx-verification.sh`. The helper requires the local path to be Git-ignored, uses a mode-0700 directory, writes mode-0600 config/password files, and reads the AMI password with terminal echo disabled. The verifier confines config, password, and detailed result paths to `.local/real-pbx-verification/`; it refuses files outside that directory or files with group/other permissions.

The verifier reuses the application Asterisk provider and network boundary and is limited to AMI login/logoff, `CoreSettings`, `CoreShowChannels`, passive event observation, and a reconciliation snapshot. It does not send originate, redirect, hangup, reload, CLI-command, or configuration-write actions. Console output contains only bounded PASS/FAIL information; target metadata, username, password, raw AMI frames, channel identities, call details, and the detailed result remain local. CI only syntax-checks these tools and never receives real PBX values.

The first approved real probe authenticated successfully but `CoreSettings` was denied by the AMI account permission mask. The verifier disconnected cleanly and performed no PBX change. The project classifies permission-like AMI failures as bounded `PERMISSION_DENIED` so operators can stop at the authorization boundary instead of exposing raw manager responses or broadening permissions blindly. After the dedicated account was corrected to the documented least-privilege categories, the bounded real gate passed login, discovery, initial/final channel snapshots, passive normalized live events during a normal test call, reconciliation, and clean disconnect without changing PBX configuration.

## Phase 2 Task 13 telephony-state review

The telephony state engine is an internal in-memory consumer and creates no PBX connection of its own. It subscribes before provider runtime start, uses only normalized provider events and the already-minimized channel snapshots, and does not expose state through HTTP or WebSocket. Browser count therefore cannot increase PBX work or access raw AMI frames.

AMI ordering metadata contains only process-local connection generations and frame sequence numbers. It carries no credential, network address, caller identity, or raw provider payload. The reducer keeps only channel identity/name, linked ID, state, bridge ID, update times, deterministic call grouping, revision, and synchronization freshness. Its event journal is bounded; unsafe overflow or reconnect boundaries fail closed by waiting for a safe authoritative snapshot rather than manufacturing current state. Runtime profile replacement/removal clears the corresponding state so data is not carried between provider instances.
