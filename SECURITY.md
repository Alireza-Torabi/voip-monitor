...
Future provider connections need an explicit SSRF and monitoring-network policy, including private and loopback addresses, link-local and metadata-service ranges, DNS rebinding, and allowed network scope. The present phase intentionally permits valid private addresses as stored data. Browser interaction tests use a local DOM simulation to check secret clearing; production HTTPS deployment and PBX connectivity remain unimplemented.
