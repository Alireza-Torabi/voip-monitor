...
export class NetworkBoundaryError extends Error {
  ...
  constructor(reason: NetworkBlockReason) {
    super(`Network target rejected: ${reason}`);
...
export function validHostSyntax(value: string): boolean {
...
/**
 * Validate a service target after one DNS resolution step.
 * Private RFC1918/ULA addresses are intentionally allowed because managed
 * infrastructure normally lives on private networks. Loopback, link-local,
 * multicast, metadata, broadcast, and unspecified targets are rejected.
 * Real transports must connect to one of the already-validated resolved
 * addresses rather than resolving again.
 */
